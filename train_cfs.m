function [erms,w_cfs] = train_cfs(training_data,m_cfs,lambda_cfs)
    ...
    %save mu_cfs.mat means;
    %save s_cfs.mat variances;
    ...
    % calculate w vector using regularized least square solution
    I = eye(length(phi(1,:)));
    w_cfs = (phi' * phi + lambda * I)\(phi' * target);
    
    %save W_cfs.mat w_cfs;
    
    % calculate error function and report rms error
